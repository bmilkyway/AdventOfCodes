    ...
    public class Day6
    {
        ...
        private class Instruction
        {
            ...
            public Instruction(string instructionLine)
            {
                ...
                startX = instructionSplit[0] == "turn" ? int.Parse(instructionSplit[2].Split(',').First()) : int.Parse(instructionSplit[1].Split(',').First());
                startY = instructionSplit[0] == "turn" ? int.Parse(instructionSplit[2].Split(',')[1]) : int.Parse(instructionSplit[1].Split(',').Last());
        ...
        private bool[,] flashStates = new bool[1000, 1000];
        private int[,] flashVolumes = new int[1000, 1000];
        private string[] commands = new string[] { "turn on", "turn off", "toggle"};
        ...
        private void ReadingFile()
        {
            ...
            {
                StreamReader sr = new StreamReader("InputFiles/day6.txt");
        ...
        private int executeInstruction(int partValue)
        {
            int count = 0;
            int flashVolume = 0;
            foreach (var instruct in instructions)
            {
                if (instruct.command.Equals(commands[0]))
                {
                    for (int x = instruct.startX; x < instruct.endX+1; x++)
                    {
                        for (int y = instruct.startY; y < instruct.endY+1; y++)
                        {
                            flashVolumes[x, y]++;
                            flashStates[x,y] = true;
                        }
                    }
                }
                else if(instruct.command.Equals(commands[1]))
                {
                    for (int x = instruct.startX; x < instruct.endX+1; x++)
                    {
                        for (int y = instruct.startY; y < instruct.endY+1; y++)
                        {
                            if (flashVolumes[x,y]>0)flashVolumes[x, y]--;
                            flashStates[x, y] = false;
                        }
                    }
                }
                else
                {
                    for (int x = instruct.startX; x < instruct.endX+1; x++)
                    {
                        for (int y = instruct.startY; y < instruct.endY+1; y++)
                        {
                            ...
                            if (flashStates[x, y]==true) flashStates[x, y] = false;
                            else flashStates[x, y] = true;
                        }
                    }
                }
            }
            ...
            for (int i = 0; i <1000; i++)
            {
                ...
                {
                    flashVolume += flashVolumes[i, y];
                    if (flashStates[i,y] == true) count++;

                }
            }

            return partValue==1?count:flashVolume;
        ...
        /// <returns></returns>
        public string Task1()=>
       String.Format("{0} Égő ég a legvégén", executeInstruction(1));
        ...
        /// <returns></returns>
        public string Task2() =>
            String.Format("{0} Az égők teljes fényereje",executeInstruction(2));
